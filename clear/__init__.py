...
def execute(name, args):
    ...
    parser.add_argument("-a", "-all", help="Removes all todo list elements", default=False, action="store_const", const=True, dest="all")
    parser.add_argument("title", help="Title of task. Deletes closest match if none are found")

    parsed = parser.parse_args(args)
    if parsed.all and (parsed.f or input(colorama.Fore.RED + "Are you sure you want to delete the ENTIRE LIST? (N/y)").lower() == "y"):
        with open("./.data/todo.json", "w") as todoFile:
            todoFile.write(json.dumps({
                "items": []
            }))
        print(colorama.Fore.RED + "Cleared!")
    elif parsed.title and (parsed.f or input(colorama.Fore.RED + "Are you sure you want to delete " + parsed.title + " from the list? (N/y)").lower() == "y"):
